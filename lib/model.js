...
function evalExpression(wordsToChange, body, expression, context) {
	var tree = jsep(expression);
	...
	if (tree.type === 'BinaryExpression') {
		...
		var left = tree.left;
		var value = this.evalExpression(tree.right, context);
		...
		if (left.type === 'Identifier') {
			...
			debug("Change word #" + idx + " to ", value);
		...
		if (left.type === 'BinaryExpression') {
			...
			if (left.operator === '#') {
				var value2 = this.evalExpression(left.right, context);
				...
				debug("Change word #" + idx + "#" + value2 + " to ", value + " => ", v);
