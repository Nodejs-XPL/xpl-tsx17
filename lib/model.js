...
class Model extends Events {
		...
		this.once('words', (words, diff) => {
			...
			async.eachSeries(units, (unit, callback) => {
				...
				async.forEachOfSeries(unit.$trees, (tree, statusName, callback) => {
					...
					this._sendUnitStatus(unit, statusName, wordsByName, false, callback)
				}, callback);
	...
	_sendUnitStatus(unit, statusName, wordsByName, testIfSame, callback) {
	...
	_createListener(unit, statusName) {
		return (wordsByName, transactionId) => {
			var tree = unit.$trees[statusName];
			...
			this._sendUnitStatus(unit, statusName, wordsByName, true);
		};
	}
}
