...
Model.prototype._requestStatus = function(device) {
	...
	this.once('words', function(words) {
		...
		console.log("Returns request status");

		async.eachSeries(units, function(unit, callback) {

			async.forEachOfSeries(unit.$trees, function(tree, status, callback) {

				console.log("Unit ", unit, " status=", status);
				callback();
			}, callback);
...
Model.prototype._sendUnitStatus = function(unit, statusName, wordsByName, testIfSame, callback) {
	...
	console.log("Exp =>", tree.$expression, wordsByName)
	...
	self.xpl.sendXplStat(trig, null, null, callback);
};
