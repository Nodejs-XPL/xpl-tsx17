...
Model.prototype._addListener = function(unit, statusName, expression) {
	...
	list.forEach(function(identifier) {
		...
		console.log("Add listener " + identifier + " " + unit.name + "/" + statusName);
		self.on(identifier, self._createListener(tree, unit, statusName));
	});
}

Model.prototype._createListener = function(tree, unit, statusName) {
	var self = this;
	return function(word, value, initPhase) {
		var context = {};
		context[word] = value;

		var val = evalExpression(tree, context);

		console.log("Exp =>", context, val)
